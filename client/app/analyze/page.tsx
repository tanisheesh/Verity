...
export default function AnalyzePage() {
  ...
  return (
    <div className="flex flex-col lg:flex-row min-h-screen">
      ...
      >
        <div className="text-[12px] font-semibold tracking-[0.2em] mb-1" style={{ color: 'var(--accent)' }}>
          VERITY
        </div>
        <div className="text-[13px] mb-6" style={{ color: 'var(--text-secondary)' }}>
          Forensic Text Analysis
        </div>

        {/* Mode Toggle */}
        <div className="flex mb-4 rounded overflow-hidden" style={{ border: '1px solid var(--border)' }}>
          <button
      ...
      {/* Right Panel - Results */}
      <div className="flex-1 p-8 overflow-y-auto" style={{ background: 'var(--bg-primary)' }}>
...
function EmptyState() {
  return (
    <div className="flex flex-col items-center justify-center h-full">
      <div className="text-[120px] leading-none mb-4" style={{ fontFamily: "'Bebas Neue', sans-serif", color: 'var(--text-muted)' }}>
        V
      </div>
...
function SingleResults({ data, originalText }: { data: AnalysisReport; originalText: string }) {
  ...
  const verdictConfig = {
    likely_truthful: { label: 'LIKELY TRUTHFUL', bg: '#00ff8820', color: '#00ff88', border: '#00ff8840' },
    uncertain: { label: 'UNCERTAIN', bg: '#ffaa0020', color: '#ffaa00', border: '#ffaa0040' },
    likely_lie: { label: 'LIKELY DECEPTIVE', bg: '#ff3b3b20', color: '#ff3b3b', border: '#ff3b3b40' },
  };
    ...
    >
      ...
      <div className="mb-6">
        <div className="flex items-baseline gap-2">
          <AnimatedScore score={data.score} color={scoreColor} />
          <span className="text-[40px]" style={{ fontFamily: "'Bebas Neue', sans-serif", color: 'var(--text-muted)' }}>
            /100
          </span>
        </div>
        <div className="text-[12px] mt-1" style={{ color: 'var(--text-secondary)' }}>
          ± {data.confidence_interval} confidence interval
        </div>
        <div
          className="inline-block px-4 py-2 rounded-full text-[13px] font-semibold tracking-[0.1em] mt-3"
          style={{ background: verdict.bg, color: verdict.color, border: `1px solid ${verdict.border}` }}
        >
          {verdict.label}
        </div>
        <p className="text-[14px] italic mt-3 max-w-md" style={{ color: 'var(--text-secondary)' }}>
          {data.summary}
        </p>
      </div>
